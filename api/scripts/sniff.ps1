	...
	if( ($Protocol -eq "all") -or ($Protocol -eq $ProtocolDesc) )
	{
		if( ($Port -eq "all") -or ($Port -eq $sourcePort) -or ($Port -eq $destPort) )
		{
			...
			#if( $ScanIP -eq $SourceIp -and $ScanIP -eq $DestinationIP )
			{
				...
				{
					"ICMP"	{
							Write-Host "Type: $ICMPType"
							Write-Host "Code: $ICMPCode"
							break
						}
					"IGMP"	{
							Write-Host "Type: $IGMPType"
							break
						}
					"TCP"	{
							Write-Host "Sequence: $SequenceNumber"
							Write-Host "Window: $TCPWindow"
							Write-Host "Service: $serviceDesc"
							break
						}
					"UDP"	{
							Write-Host "Service: $serviceDesc"
